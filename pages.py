...
conn = pymssql.connect(
    ...
    port='1433',
    user='SA',
    password='aCecf467890',
    database='master',
    autocommit=True)
print(conn)
cursor = conn.cursor()
cursor.execute('use 学生管理')
...
class grade_query_admin_page(query_page):
    ...
    def get_list(self):
        _list=multipleList(
            ...
            area=self.area,
            height=20,
            width=(100,50,30,55,75,55,40,35,55,100,65,55,55,40,40,40),
            command=self.search_double_click)
        return _list
    ...
    def add(self):
        ...
        if self.judge():
            ...
            if len(_cno)==0:
                ...
                _cno=_cno[0][0]
                try:
                    sql("insert into xxxx values('%s','%s',%s)"%(_id,_cno,_grade))
            ...
            if len(sql("select sno from xsxx where sno='%s'"%_id))==0:
                ...
                if len(_cno)==0:
                    ...
                    if len(sql("select * from xxxx where sno='%s' and cno='%s'"%(_id,_cno)))==0:
                        ...
                        sql("update xxxx set grade=%s where sno='%s' and cno='%s'"%(_grade,_id,_cno))
                        self.data=self.get_data()
                        self.search1()
            ...
            if len(sql("select sno from xsxx where sno='%s'"%_id))==0:
                ...
                if len(_cno)==0:
                    ...
                    if len(sql("select * from xxxx where sno='%s' and cno='%s'"%(_id,_cno)))==0:
                        ...
                        sql("delete from xxxx where sno='%s' and cno='%s'"%(_id,_cno))
                        self.data=self.get_data()
                        self.search2()
...
class stu_rcd_mtnc_page(stu_mtnc_page):
    ...
    def add(self):
        ...
        if self.judge():
            try:
                sql("insert into xsxx values('%s','%s','%s',%s,%s)"%(_id,_sname,_sex,_class,_age))
            except Exception:
    ...
    def edit(self):
        ...
        self.search()
        if self.judge():
            sql("update xsxx set sname='%s',sex='%s',class=%s,age=%s where sno='%s'"%(_sname,_sex,_class,_age,_id))
            self.data=self.get_data()
            self.search()
